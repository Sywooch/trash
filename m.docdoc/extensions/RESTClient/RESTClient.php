...
class RESTClient extends CComponent
{
    ...
    public function get($uri, $params = array(), $format = NULL, $caching = false, $cacheTime = 0, $cacheError = 600)
    {
        if ($params) {
            $uri .= '?' . (is_array($params) ? http_build_query($params) : $params);
        }

		if ($caching && Yii::app()->params->isRestCache) {
            $key = $this->rest_server . $uri . $format;
            if ($result = $this->_get_cache($key)) {
                return $result;
            } else {
                if($data = $this->_call('get', $uri, NULL, $format)){
                    $this->_set_cache($key, $data, $cacheTime);
                }
                else {
                    $this->_set_cache($key, $data, $cacheError);
                }
    ...
    {
        Yii::app()->cache->set($key, serialize($data), $time);
    }
