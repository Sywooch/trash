...
// Если класс Метро уже определен, то выходим
    ...
    als.Metro.data = {
         ...
         */
        url			: function(city){
            return '/js/metro/' + city + '/data.js?'+Math.random();
        }
    };
        ...
        initSearch	: function(){
            ...
            self.filter.autocomplete({
                ...
                select: function( event, ui ) {
                    ...
                    $('input[name=stations]').val( ui.item.value );
...
//		self.node.trigger('mapchange');
        ...
        load	: function(callback){
            var self = this;
            var mdata = als.Metro.data;

            if (!mdata.cityData[self.city]) {
                if (mdata.loadCallbacks === null) {
                    mdata.loadCallbacks = [ callback ];
                    $.getJSON(mdata.url(self.city), function (data) {
                        mdata.cityData[self.city] = data;
                        $.each(mdata.loadCallbacks, function (i, callback) {
                            if (callback) callback();
                        });
                        mdata.loadCallbacks = null;
                    });
                } else {
                    mdata.loadCallbacks.push(callback);
                }
            } else {
                if(callback) callback();
            }
