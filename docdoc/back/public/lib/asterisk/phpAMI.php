...
class phpAMI{
	...
	function loginByKey(){		
		if($this->openSock()){
			...
			$key = md5($response['Info']['Challenge'].$this->SECRET);
			$this->send("Login",array("AuthType"=>"MD5","Username"=>$this->USER,"Key"=>$key));
			$response=$this->readEnd();
			if($response["Response"]=="Success"){
				$response2=$this->readEnd();
				$this->PRIVILEGE=$response2["Privilege"];
	 ...
	 * @return array ver eventSimple()
	 */
	function aTxfer($channel,$context,$exten,$priority,$timeout){
		return $this->eventSimple("Atxfer",array("Channel"=>$channel,"Exten"=>$exten,"Context"=>$context,"Priority"=>$priority,"Timeout"=>$timeout));
	}
	/**
...
/*		if(is_null($wm)){
	...
	protected function saveLog(){
		...
		$sql = "INSERT INTO log_ami (request,response) VALUES ('$request','$response')";
