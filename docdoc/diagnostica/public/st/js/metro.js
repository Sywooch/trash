...
// Если класс Метро уже определен, то выходим
		...
		als.Metro.data = {
			 ...
			 */
			url: function (city) {
				return '/st/js/metro/' + city + '/data.js?' + Math.random();
			}
		};
		...
		als.Metro.prototype = {
			...
			initActionButtons: function () {
				...
				$('.als_metro_select_inside').click(function () {
					...
					self.toggleSelectedStations(self.cityData.inside, function () {
				...
				$('.als_metro_select_circle').click(function () {
					var $Wrapper = $(this).parent('span');
					self.toggleSelectedStations(self.cityData.circle, function () {
						$Wrapper.toggleClass('active')
					});
					self.triggerAll();
				});
			...
			initSearch: function () {
				...
				self.filter = $('.search_input_geo');

				var m = $('#geoDataJson');
				if (m.length > 0) {
					m = m.val();
					m = JSON.parse(m);
					m = $.map(m, function (item) {
						return {
							value: item.value,
							label: item.label,
							id: item.id
				...
				self.filter.autocomplete({
					...
					source: function (request, response) {
						var term = $.ui.autocomplete.escapeRegex(request.term), startsWithMatcher = new RegExp("^" + term, "i"), startsWith = $.grep(m, function (value) {
								return startsWithMatcher.test(value.label || value.value || value);
							}), containsMatcher = new RegExp(term, "i"), contains = $.grep(m, function (value) {
					...
					select: function (event, ui) {
						$('input[name=geoValue]').val(ui.item.id);
				...
				self.filter2 = $('.metro_filter');
				self.filter2.autocomplete({
					minLength: 0,
					//source: m,
					...
					focus: function (event, ui) {
						self.filter2.val(ui.item.label);
						return false;
					},
					select: function (event, ui) {
						self.filter2.val('');
						self.getStationById(ui.item.id).toggle(true);
						self.node.trigger('mapchange');

						$('input[name=geoValue]').val(ui.item.value);
						$(this).val(ui.item.label);
						return false;
					},
