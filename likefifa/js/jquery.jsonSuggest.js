		...
		return this.each(function() {
			...
			*/
			function setHoverClass(el) {
				$('li a', results).removeClass('focus');
				if (el) {
					$('a', el).addClass('focus');
				}
			...
			*/
			function buildResults(resultObjects, filterTxt) {
				...
				$(results).append(resultInner).find(".ui-corner-all:first").addClass("focus");
				
				if ($('li', results).length > 0) {
					resSuggestEmpty = false;
					currentSelection = $(results).find("li:first");
					$(results).show();
					$(resultInner).css('height', 'auto');
					
					if ($(resultInner).height() > settings.maxHeight) {
						$(resultInner).css({'overflow': 'auto', 'height': settings.maxHeight + 'px'});
					}
					var apiSuggest = $(resultInner).jScrollPane({verticalGutter: 5}).data('jsp');
				}
				else
					resSuggestEmpty = true;
			}
			...
			*/			
			function runSuggest(e) {	
				...
				if (settings.data && settings.data.length) {
					...
					var text = this.value;
					/*$(results).html('<li class="ui-menu-item ajaxSearching"><a class="ui-corner-all">Поиск...</a></li>').
