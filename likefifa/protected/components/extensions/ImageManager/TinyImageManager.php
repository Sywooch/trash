...
class TinyImageManager {
	...
	function TinyImageManager() {
		...
		$this->dir = array(
			'images'	=> realpath(ROOT_PATH.Yii::app()->params['imagesDir']),
			'files'		=> realpath(ROOT_PATH.Yii::app()->params['filesDir'])
		);
	 ...
	 *
	 * @param string $requestDirectory Запрашиваемая папка (относительно imagesDir или filesDir)
	...
	function DirStructure($type, $innerDirs='first', $currentDir='', $level=0) {
		...
		if($innerDirs == 'first') {
			...
			$ret = '';
			if($innerDirs == false) return 'Неверно задана корневая директория ('.Yii::app()->params['imagesDir'].')';
			...
			while (false !== ($file = readdir($handle))) {
				if ($file != "." && $file != "..") {
					...
					if(is_file($dir.'/'.$file) ) {
						...
						$link = str_replace(array('/\\','//','\\\\','\\'),'/', '/'.str_replace(realpath(ROOT_PATH),'',realpath($dir.'/'.$file)));
						$path = pathinfo($link);
						$path = $path['dirname'];
	...
	function ShowDir($dir, $type) {
		...
		foreach ($dir as $v) {
			...
			if($v['general']['width'] > Yii::app()->params['tinyMCE']['widthToLink'] || $v['general']['height'] > Yii::app()->params['tinyMCE']['heightToLink']) {
				if($v['general']['width'] > $v['general']['height']) {
					$middle_thumb = $this->GetThumb($v['general']['path'],$v['general']['md5'],$v['general']['filename'],0,Yii::app()->params['tinyMCE']['widthToLink'],0);
				} else {
					$middle_thumb = $this->GetThumb($v['general']['path'],$v['general']['md5'],$v['general']['filename'],0,0,Yii::app()->params['tinyMCE']['heightToLink']);
				}
				list($middle_width, $middle_height) = getimagesize(ROOT_PATH.$middle_thumb);
				$middle_thumb_attr = 'fmiddle="'.$middle_thumb.'" fmiddlewidth="'.$middle_width.'" fmiddleheight="'.$middle_height.'" fclass="'.Yii::app()->params['tinyMCE']['classLink'].'" frel="'.Yii::app()->params['tinyMCE']['relLink'].'"';
			} else {
				$middle_thumb = '';
				$middle_thumb_attr = '';
			}
   ...
   <table class="imageBlock0" cellpadding="0" cellspacing="0" filename="'.$v['general']['filename'].'" fname="'.$v['general']['name'].'" type="'.$type.'" ext="'.strtoupper($v['general']['ext']).'" path="'.$v['general']['path'].'" linkto="'.$v['general']['link'].'" fsize="'.$v['general']['size'].'" fsizetext="'.$this->bytes_to_str($v['general']['size']).'" date="'.date('d.m.Y H:i',$v['general']['date']).'" fwidth="'.$v['general']['width'].'" fheight="'.$v['general']['height'].'" md5="'.$v['general']['md5'].'" '.$middle_thumb_attr.'><tr><td valign="bottom" align="center">
	...
	function GetThumb($dir, $md5, $filename, $mode, $width=100, $height=100) {
		...
		$isfile = (strpos($dir, Yii::app()->params['filesDir']) === 0 ? true : false);
		...
		if ($isfile)
		{
			...
			$url = '/'.ltrim(substr($server_url, strlen(ROOT_PATH)), '/');
	...
	function DelFolder($pathtype, $path) {
		...
		if(realpath($path.'/') == realpath(ROOT_PATH.Yii::app()->params['imagesDir'].'/')) {
