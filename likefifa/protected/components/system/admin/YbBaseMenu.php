 ...
 */
abstract class YbBaseMenu extends CMenu {
	/**
	 *### .getDividerCssClass()
	 *
			...
			foreach ($items as $item) {
				...
				if (isset($item['divider'])) {
					...
					if (isset($item['items']) && !empty($item['items'])) {
						...
						$this->controller->widget('likefifa\components\system\admin\YbDropdown', $dropdownOptions);
					}
	...
	protected function renderMenuItem($item) {
		...
		if (isset($item['items']) && !empty($item['items'])) {
			...
			if (isset($item['linkOptions']['class'])) {
				$item['linkOptions']['class'] .= ' dropmenu';
			} else {
				$item['linkOptions']['class'] = 'dropmenu';
			}

			//$item['linkOptions']['data-toggle'] = 'dropdown';
			$item['label'] .= ' <span class="chevron closed"></span>';
		}
