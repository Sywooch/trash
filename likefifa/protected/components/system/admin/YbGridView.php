 ...
 * @version 0.3
 */
class YbGridView extends TbExtendedGridView
{
	public $ajaxUpdate = false;
	public $type = 'striped bordered condensed';
	public $filterSelector = '{filter}, #filter-form';
	public $template = '{items}{exportbuttons}{pager}';
	...
	public $libPath = 'application.vendors.phpoffice.phpexcel.Classes.PHPExcel'; //the path to the PHP excel lib
	...
	public function init()
	{
		...
		foreach($this->columns as $i=>$column) {
			if(isset($column['visible']) &&  !is_bool($column['visible']) && $column['visible'] == 'excel' && $this->grid_mode != 'export')
			{
		...
		foreach ($this->columns as $n => $column) {
			if ($column instanceof CLinkColumn) {
				if ($column->labelExpression !== null) {
					$value =
						$column->evaluateExpression(
							$column->labelExpression,
				...
				$value = "";
			} //Dont know what to do with buttons
	...
	public function run()
	{
		if ($this->grid_mode == 'export') {
			...
			if ($this->autoWidth) {
				foreach ($this->columns as $n => $column) {
					$this->objPHPExcel->getActiveSheet()->getColumnDimension($this->columnName($n + 1))->setAutoSize(
						true
					);
				}
			}
			...
			} else //output to browser
			{
				...
				header(
					...
					$this->mimeTypes[$this->exportType]['extension'] .
					'"'
				);
		...
		foreach ($this->exportButtons as $key => $button) {
			...
			if (key_exists('query', $url)) {
				$content[] =
					CHtml::link(
						$item['caption'],
						'?' . $url['query'] . '&exportType=' . $type . '&' . $this->grid_mode_var . '=export',
				...
				$content[] =
					CHtml::link(
						$item['caption'],
						'?exportType=' . $type . '&' . $this->grid_mode_var . '=export',
						['class' => 'btn btn-success']
					);
			}
		}
