				...
				<li>
					...
					<ul>
						<?php foreach (AreaMoscow::model()->with('districts')->ordered()->findAll() as $area): ?>
							<li>
								&mdash; <a href="<?php echo $this->forSalons()->createAreaUrl(
									$area
								); ?>"><?php echo $area->name; ?></a>
								<ul>
									<?php foreach ($area->districts as $district): ?>
										<li>&mdash; <a href="<?php echo $this->forSalons()->createSearchUrl($specialization, $service, false, array(), $area, array($district)); ?>"><?php echo $district->name; ?></a></li>
									<?php endforeach; ?>
								</ul>
							</li>
						<?php endforeach; ?>
				...
				<li>
					...
					<ul>
						<?php foreach (UndergroundStation::model()->ordered()->findAll() as $station): ?>
							<li>&mdash; <a href="<?php echo $this->forSalons()->createSearchUrl($specialization, $service, false, array($station)); ?>"><?php echo $station->name; ?></a></li>
						<?php endforeach; ?>
					</ul>
				</li>
				<?php } else { ?>
					<li>
						...
						<ul>
							<?php foreach (Yii::app()->activeRegion->getModel()->activeCities as $city) { ?>
								<li>
									&mdash; <a href="<?php echo $this->forSalons()->createCityUrl(
										$city->rewrite_name,
										$specialization
