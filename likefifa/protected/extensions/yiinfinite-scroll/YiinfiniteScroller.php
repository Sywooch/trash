...
require_once(__DIR__ . DIRECTORY_SEPARATOR . 'YiinfiniteScrollerException.php');

class YiinfiniteScroller extends CBasePager {

    public $contentSelector = '#content';
    public $navigationLinkText = 'next';
	public $callback = 'js:function() {console.log(this);}';
    ...
    public function run() {
        ...
        if($this->currentPageDoesntExists()) {
            throw new YiinfiniteScrollerException(404, 'Page not found');
        }
    }
    ...
    public function __set($name, $value) {
        ...
        } else if(array_key_exists($name, $this->_default_options)) {
			return $this->_default_options[$name] = $value;
		}
    ...
    private function createInfiniteScrollScript() {
        Yii::app()->clientScript->registerScript(
            uniqid(),
            "$('{$this->contentSelector}').infinitescroll(".$this->buildInifiniteScrollOptions().", ".CJavaScript::encode($this->callback).");"
        );
    }
