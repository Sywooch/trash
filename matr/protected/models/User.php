...
class User extends CActiveRecord
{
	...
	public function rules()
	{
		return array(
			...
			array('balance_personal, balance_shop, type, group_number', 'numerical'),
			array('email, name, perfect, payer', 'length', 'max' => 128),
			array('password', 'length', 'max' => 40),
			array('skype, city', 'length', 'max' => 64),
			array('phone', 'length', 'max' => 20),
			array(
				'id, email, password, name, skype, phone, city, balance, parent_id, created, balance_personal,
				balance_shop, perfect, payer',
				'safe',
				'on' => 'search'
			),
	...
	public function attributeLabels()
	{
		return array(
			...
			'perfect'           => 'Кошелек PerfectMoney',
			'payer'             => 'Кошелек Payeer',
		);
	}
	...
	public function getBalance()
	{
		...
		$criteria->condition = "t.group_number = {$this->group_number}";
		foreach (User::model()->findAll($criteria) as $model) {
			$balance = $balance + $model->balance_personal;
		}
	...
	public function search()
	{
		...
		$criteria->compare('perfect', $this->perfect, true);
	...
	function sendFirstMail($password)
	{
		...
		$mail->FromName = "Русская матрешка";
	...
	public function getNewParentId()
	{
		if ($this->parent_id) {
			...
			if ($model && $model->is_run_out_childs == 0 && $model->type == $this->type) {
		...
		$criteria = new CDbCriteria;
		$criteria->condition = "t.is_active = :is_active AND t.is_run_out_childs = :is_run_out_childs AND t.type = :type";
